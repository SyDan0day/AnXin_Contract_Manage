        ...
        <el-tab-pane label="审批记录" name="approvals">
          <div class="tab-header">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
              <el-tag v-if="workflow" :type="getWorkflowStatusType(workflow.status)" size="large">
                {{ getWorkflowStatusText(workflow.status) }}
              </el-tag>
              <el-button type="primary" size="small" @click="showApprovalDialog = true" v-if="(userRole === 'user' || userRole === 'admin') && (contract.status === 'draft' || contract.status === 'pending')" class="approval-btn">
          ...
          <!-- 审批流程概览 -->
          <div v-if="workflow" class="workflow-overview" style="margin-bottom: 20px;">
              ...
              <el-timeline>
                ...
                >
                  <el-card shadow="never" :body-style="{ padding: '10px' }">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                      <div>
                        <el-tag size="small" :type="getLevelTagType(approval.level)">
                          Level {{ approval.level }}
                        </el-tag>
                        <span style="margin-left: 8px; font-weight: 500;">
                          {{ getRoleText(approval.approver_role) }}
                        </span>
                      </div>
                      <el-tag size="small" :type="getApprovalStatusType(approval.status)">
                        {{ getApprovalStatusText(approval.status) }}
                      </el-tag>
                    </div>
                    <div v-if="approval.approver?.full_name" style="margin-top: 8px; color: #666;">
                      审批人: {{ approval.approver.full_name }}
                    </div>
                    <div v-if="approval.comment" style="margin-top: 4px; color: #999; font-size: 13px;">
                      意见: {{ approval.comment }}
                    </div>
                  </el-card>
                </el-timeline-item>
          ...
          <el-table :data="approvals" v-loading="approvalsLoading">
            ...
            <el-table-column label="状态" width="100">
              <template #default="{ row }">
                <el-tag :type="getApprovalStatusType(row.status)">{{ getApprovalStatusText(row.status, row.contract_status) }}</el-tag>
              </template>
            </el-table-column>
...
import { getContractDetail, getContractDocuments, uploadDocument, deleteDocument, getPreviewToken, getContractLifecycle, updateContractStatus, archiveContract, requestStatusChange } from '@/api/contract'
import { getApprovalRecords, getContractWorkflow, createApproval } from '@/api/approval'
...
const getApprovalStatusType = (status, contractStatus) => {
  if (contractStatus === 'draft') return 'info'
  if (contractStatus === 'pending') {
    const map = { pending: 'warning', approved: 'success', rejected: 'danger' }
    return map[status] || ''
  }
  if (contractStatus === 'active') return 'primary'
  if (contractStatus === 'completed') return 'success'
  if (contractStatus === 'rejected') return 'danger'
  if (contractStatus === 'terminated') return 'info'
  return ''
}

const getApprovalStatusText = (status, contractStatus) => {
  if (contractStatus === 'draft') return '草稿'
  if (contractStatus === 'pending') {
    const map = { pending: '待审批', approved: '已批准', rejected: '已拒绝' }
    return map[status] || status
  }
  if (contractStatus === 'active') return '已生效'
  if (contractStatus === 'completed') return '已完成'
  if (contractStatus === 'rejected') return '已拒绝'
  if (contractStatus === 'terminated') return '已终止'
  return status
}

const getWorkflowStatusType = (status) => {
  const map = { pending: 'warning', approved: 'success', rejected: 'danger', completed: 'success' }
  return map[status] || ''
}

const getWorkflowStatusText = (status) => {
  const map = { pending: '审批中', approved: '已批准', rejected: '已拒绝', completed: '已完成' }
  return map[status] || status
}

const getTimelineItemType = (status) => {
  const map = { pending: 'primary', approved: 'success', rejected: 'danger' }
...
const loadApprovals = async () => {
  ...
  try {
    const [approvalData, workflowData] = await Promise.all([
      getApprovalRecords(contractId.value),
      getContractWorkflow(contractId.value).catch(() => null)
    ])
